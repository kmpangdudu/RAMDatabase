...
--              再运用 biz-login
-- Reversion:   2017-08-23, using Biz-logic ver2. added "Program" Stuffs
-- =============================================================================================================
CREATE FUNCTION [dbo].[F_ram]
(
...
--Program
p_etlloadid INT,
p_ResourceAgencyNum NVARCHAR(20),
p_publicname NVARCHAR(2000), 
p_AgencyDescription NVARCHAR(2000), 
p_TaxonomyLevelNameID INT, 
p_ParentAgencyNum NVARCHAR(20),
p_PhysicalAddress  NVARCHAR(2000), 
p_PhysicalCityID INT,
p_PhysicalProvinceID INT,
p_PhysicalCountry NVARCHAR(20),
p_PhysicalPostalcode NVARCHAR(20),
p_PhysicalAddressIsPrivate NVARCHAR(20),
p_latitude NUMERIC(18,10), 
p_longitude  NUMERIC(18,10),
p_HoursOfOperation NVARCHAR(2000),
p_Phone  NVARCHAR(20),
p_WebsiteAddress NVARCHAR(2000),
p_Eligibility NVARCHAR(2000),
p_DisabilitiesAccess NVARCHAR(2000),
p_FeeStructureSource NVARCHAR(2000),
p_ApplicationProcess NVARCHAR(2000),
p_DocumentsRequired NVARCHAR(2000),
p_LanguagesOfferedList NVARCHAR(2000),
p_LanguageOfRecord NVARCHAR(20),
p_Hours NVARCHAR(2000),
p_CustomEligibilitybyAge  NVARCHAR(2000),
p_CoverageArea  NVARCHAR(2000),
p_Coverage NVARCHAR(MAX),
p_NormalWaitTime NVARCHAR(2000),
p_ConnectsToProgramNum NVARCHAR(20),
p_ConnectsToSiteNum NVARCHAR(20),
p_changedDate DATETIME,
...
-- Idenify coming record's TaxonomyNameLeavl
-- 1:Agency;   2:Site;    3:Program;     4:Program@Site
...
--Program
DECLARE @p_etlloadid INT;
DECLARE @p_ResourceAgencyNum NVARCHAR(20);
DECLARE @p_publicname NVARCHAR(2000); 
DECLARE @p_AgencyDescription NVARCHAR(2000); 
DECLARE @p_TaxonomyLevelNameID INT; 
DECLARE @p_ParentAgencyNum NVARCHAR(20);
DECLARE @p_PhysicalAddress  NVARCHAR(2000); 
DECLARE @p_PhysicalCityID INT;
DECLARE @p_PhysicalProvinceID INT;
DECLARE @p_PhysicalCountry NVARCHAR(20);
DECLARE @p_PhysicalPostalcode NVARCHAR(20);
DECLARE @p_PhysicalAddressIsPrivate NVARCHAR(20);
DECLARE @p_latitude NUMERIC(18,10); 
DECLARE @p_longitude  NUMERIC(18,10);
DECLARE @p_HoursOfOperation NVARCHAR(2000);
DECLARE @p_Phone  NVARCHAR(20);
DECLARE @p_WebsiteAddress NVARCHAR(2000);
DECLARE @p_Eligibility NVARCHAR(2000);
DECLARE @p_DisabilitiesAccess NVARCHAR(2000);
DECLARE @p_FeeStructureSource NVARCHAR(2000);
DECLARE @p_ApplicationProcess NVARCHAR(2000);
DECLARE @p_DocumentsRequired NVARCHAR(2000);
DECLARE @p_LanguagesOfferedList NVARCHAR(2000);
DECLARE @p_LanguageOfRecord NVARCHAR(20);
DECLARE @p_Hours NVARCHAR(2000);
DECLARE @p_CustomEligibilitybyAge  NVARCHAR(2000);
DECLARE @p_CoverageArea  NVARCHAR(2000);
DECLARE @p_Coverage NVARCHAR(MAX);
DECLARE @p_NormalWaitTime NVARCHAR(2000);
DECLARE @p_ConnectsToProgramNum NVARCHAR(20);
DECLARE @p_ConnectsToSiteNum NVARCHAR(20);
DECLARE @p_changedDate DATETIME;
...
IF @TaxonomyNameLeavlID = 3 
		...
		--program
		SELECT  
					@p_etlloadid                 =  etlloadid               ,
					@p_ResourceAgencyNum         =  ResourceAgencyNum       ,
					@p_publicname                = ISNULL(PublicName,'')              ,
					@p_AgencyDescription         = ISNULL(AgencyDescription,'')       ,
					@p_TaxonomyLevelNameID       =  TaxonomyLevelNameID     ,
					@p_ParentAgencyNum           =  ParentAgencyNum         ,
					@p_PhysicalAddress           = ISNULL(PhysicalAddress1,'')+' '+ISNULL(PhysicalAddress2,'') ,          
					@p_PhysicalCityID            = ISNULL(PhysicalCityID,0)          ,
					@p_PhysicalProvinceID        = ISNULL(PhysicalProvinceID,0)      ,
					@p_PhysicalCountry           = ISNULL(PhysicalCountry,''),
					@p_PhysicalPostalcode        = ISNULL(PhysicalPostalCode,'')      ,
					@p_PhysicalAddressIsPrivate  =  PhysicalAddressIsPrivate,
					@p_latitude                  = ISNULL(latitude,0.0)                ,
					@p_longitude                 = ISNULL(longitude,0.0)               ,
					@p_HoursOfOperation          = ISNULL(HoursOfOperation,'')        ,
					...
					@p_WebsiteAddress            = ISNULL(WebsiteAddress,'')          ,
					@p_Eligibility               = ISNULL(Eligibility,'')             ,
					@p_DisabilitiesAccess        = ISNULL(DisabilitiesAccess,'')      ,
					@p_FeeStructureSource        = ISNULL(FeeStructureSource,'')      ,
					@p_ApplicationProcess        = ISNULL(ApplicationProcess,'')      ,
					@p_DocumentsRequired         = ISNULL(DocumentsRequired,'')        ,
					@p_LanguagesOfferedList      = ISNULL(LanguagesOfferedList,'')     ,
					@p_LanguageOfRecord          = ISNULL(LanguageOfRecord,'')        ,
					@p_Hours                     = ISNULL(WorkHours,'')               ,              
					@p_CustomEligibilitybyAge    = ISNULL(CustomEligibilitybyAge,'')  ,
					@p_CoverageArea              = ISNULL(CoverageArea ,'')  ,
					@p_Coverage                  = ISNULL(Coverage ,'')  ,
					@p_NormalWaitTime            = ISNULL( NormalWaitTime ,'')    ,
					@p_ConnectsToProgramNum      =  ConnectsToProgramNum    ,
					@p_ConnectsToSiteNum         =  ConnectsToSiteNum       ,
					@p_changedDate               = ISNULL(changedDate,getdate())               
				FROM [dbo].RAMRAW  WHERE ETLLOADID = @ETLLOADID   -- 他自己就是Program
...
ELSE -- @TaxonomyNameLeavlID = 4  coming record is a Program@Site
		...
		-- program
			...
			@p_publicname , 
			@p_AgencyDescription , 
			@p_TaxonomyLevelNameID , 
			@p_ParentAgencyNum ,
			@p_PhysicalAddress  , 
			@p_PhysicalCityID ,
			@p_PhysicalProvinceID ,
			@p_PhysicalCountry,
			@p_PhysicalPostalcode ,
			@p_PhysicalAddressIsPrivate ,
			@p_latitude , 
			@p_longitude  ,
			@p_HoursOfOperation ,
			@p_Phone  ,
			@p_WebsiteAddress ,
			@p_Eligibility ,
			@p_DisabilitiesAccess ,
			@p_FeeStructureSource ,
			@p_ApplicationProcess ,
			@p_DocumentsRequired ,
			@p_LanguagesOfferedList ,
			@p_LanguageOfRecord ,
			@p_Hours ,
			@p_CustomEligibilitybyAge  ,
			@p_CoverageArea  ,
			@p_Coverage      ,
			@p_NormalWaitTime ,
			@p_ConnectsToProgramNum ,
			@p_ConnectsToSiteNum ,
			@p_changedDate ,
